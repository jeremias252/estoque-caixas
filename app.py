...
# Nomes de arquivos independentes do setor de Torres
ARQUIVO_ESTOQUE = "estoque_caixas.csv"
ARQUIVO_HISTORICO = "historico_caixas.csv"
...
def carregar_dados():
    if os.path.exists(ARQUIVO_ESTOQUE):
        ...
        # Modelos que possuem variação de cor
        modelos_com_cor = [
            "CXP01T", "CXP01", "CX04S", "CX34ABS", "CX44", 
            "CX23ABS", "CX01S", "CX02S", "CX03S", "CXEP02", 
            "CXEP03", "CP01A", "RE04FN", "RE06FN", "CX02Q", "CX02RN"
        ]
        cores = ["Branco", "Preto", "Cinza"]
        
        # Gera os modelos com cores
        itens = [f"{modelo} - {cor}" for modelo in modelos_com_cor for cor in cores]
        
        # Adiciona o modelo especial que não tem cor
        itens.append("CX56")
...
def exibir_estoque_premium(df_base, termo_busca=""):
    ...
    def extrair_cor(nome):
        ...
        return "Padrão" # Para a caixa CX56 que não tem cor
...
if not (acesso_marcello or acesso_coord):
    ...
    with abas[0]:
        ...
        with st.form("form_novo", clear_on_submit=True):
            novo_nome = st.text_input("Nome do produto (Recomendado adicionar cor)")
    ...
    if acesso_coord:
        with abas[3]:
            ...
            st.write("Exporte os relatórios gerenciais consolidados do setor de CAIXAS.")
            ...
            st.download_button(
                ...
                file_name=f"fechamento_caixas_{datetime.now().strftime('%d-%m-%Y')}.txt",
                mime="text/plain",
            ...
            with col_down1:
                ...
                st.download_button(
                    ...
                    data=csv_estoque,
                    file_name=f"estoque_caixas_{datetime.now().strftime('%d-%m-%Y')}.csv",
                    mime="text/csv",
                    use_container_width=True
                )
